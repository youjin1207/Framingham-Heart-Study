...
load("../data/LatentCate.RData")
outcome0 = matrix(0, 500, 200); outcome1 = matrix(0, 500, 200)
outcome2 = matrix(0, 500, 200); outcome3 = matrix(0, 500, 200)
phi0 = c(); phi1 = c(); phi2 = c(); phi3 = c()
Zpower = rep(0,4); Ppower = rep(0,4)
for(mm in 1:500){
  outcome0[mm,] = LatentCate[[mm]][[1]][1,]
  outcome1[mm,] = LatentCate[[mm]][[1]][2,]
  outcome2[mm,] = LatentCate[[mm]][[1]][3,]
  outcome3[mm,] = LatentCate[[mm]][[1]][4,]
  
  phi0[mm] = LatentCate[[mm]][[2]][1,1]
  phi1[mm] = LatentCate[[mm]][[2]][2,1]
  phi2[mm] = LatentCate[[mm]][[2]][3,1]
  phi3[mm] = LatentCate[[mm]][[2]][4,1]
  
  Zpower = Zpower + (LatentCate[[mm]][[2]][c(1:4), 2] <= alpha) / 500
  Ppower = Ppower + (LatentCate[[mm]][[2]][c(1:4), 3] <= alpha) / 500
}
...
### summary table
latentpeer_summary = matrix(NA, nrow = 4, ncol = 3)

rownames(latentpeer_summary) = c("rho=0.0", "rho=0.3", "rho=0.4", "rho=0.5")
colnames(latentpeer_summary) =  c("95% CI coverage",
                                  "% of p-values(z)<=0.05", 
                                  "% of p-values(permuation) <=0.05")
latentpeer_summary[,1] = count / 500
latentpeer_summary[,2] = Zpower*100
latentpeer_summary[,3] = Ppower*100

latentpeer_summary <- as.data.frame(latentpeer_summary)
print(xtable(latentpeer_summary , digits = 2, row.names = TRUE))



pdf("../figures/latentcate_hist.pdf", width = 15, height = 8)
par(mfrow = c(1,1),   mar = c(7,10,5,3),  cex.lab = 3, 
    cex.main = 3, cex.axis = 2, tcl = 0.5)
plot(density(phi0), 
     xlab = expression(Phi),
     main = expression(paste("Distribution of ", Phi, " under latent variable dependence")), 
...
legend("topright", c(expression(paste(rho, " = 0.0", sep="")), expression(paste(rho, " = 0.3", sep="")),
                     expression(paste(rho, " = 0.4", sep="")), expression(paste(rho, " = 0.5", sep=""))),
